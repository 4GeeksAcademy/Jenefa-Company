...
def register_user(payload: UserCreate) -> UserWithProfile:
    ...
    # Atomic-ish TinyDB transaction: roll back user if profile insert fails.
    db.insert_user(user_doc)
    try:
        db.insert_profile(profile_doc)
    except Exception:
        db.delete_user(user_id)
        raise

    return UserWithProfile(
        **_public_user(user_doc).model_dump(),
        profile=_public_profile(profile_doc),
    )
...
def update_user(
    *,
    ...
    updated = db.update_user(target_user_id, updates)
    if updated is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
...
def update_my_profile(
    *,
    ...
    if not updates:
        public = _public_profile(existing)
        assert public is not None
        return public

    updated = db.update_profile(current_user["id"], updates)
    if updated is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Profile not found")
