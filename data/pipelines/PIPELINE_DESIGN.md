...
The production cadence is every Monday at 07:00 in the reporting deployment's
configured timezone. The scheduler invokes the same standalone entrypoint used
for recovery and local validation:

```bash
python data/pipelines/pipeline.py
```

A host-level cron entry can provide the weekly trigger (the deployment should
set `TZ` to the agreed HealthCore reporting timezone):

```cron
0 7 * * 1 cd /workspaces/Jenefa-Company && /workspaces/Jenefa-Company/.venv/bin/python data/pipelines/pipeline.py >> data/eval/pipeline-cron.log 2>&1
```

The API manual recovery endpoint is `POST /services/reporting/trigger`; it
queues the same Prefect flow asynchronously. Status and output are exposed by
`GET /services/reporting/status` and `GET /services/reporting/kpis`.
